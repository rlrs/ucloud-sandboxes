...
regression-test commit; the commit column is filled after that commit exists.

Status values: `pending`, `in progress`, `complete`, or `blocked`.

| # | Priority | Status | Issue | Intended fix | Commit |
|---:|:---:|:---:|---|---|---|
| 1 | P1 | pending | Concurrent heartbeats can overwrite newer node state. | Make receipt ordering, previous-state lookup, and idle-clock derivation atomic; reject older gateway receipts. | — |
| 2 | P1 | pending | A request larger than the configured VM shape scales through `max_nodes`. | Reject or terminally classify unschedulable shapes before they contribute create demand. | — |
