...
if '--short-quantum' in sys.argv:sys.setswitchinterval(.001)
with tempfile.TemporaryDirectory() as tmp:
 ...
 for i in range(256):
  r=SandboxRoute(sandbox_id=f's{i}',node_id=f'n{i//128}',job_id=f'j{i//128}',node_url=f'http://n{i//128}',resources=ResourceQuantity(vcpu=1,memory_mb=1024),spec={'id':f's{i}','image':'python','metadata':'x'*16384},state='running',generation=1,create_operation_id='create-'+str(i),spec_hash='a'*64,node_epoch='boot',created_at=utc_now().isoformat())
 ...
 print(json.dumps({'gil_switch_seconds':sys.getswitchinterval(),'source':__import__('ucloud_sandboxes.control_plane',fromlist=['x']).__file__,'python':sys.version,'elapsed':time.monotonic()-t,'latency':{k:{'count':len(a),'p50':statistics.median(a),'p95':sorted(a)[int(len(a)*.95)],'max':max(a)} for k,a in lat.items()},'batch':s._write_batches.metrics()}),flush=True)
