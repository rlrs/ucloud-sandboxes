...
class RelayKeepaliveTests(unittest.IsolatedAsyncioTestCase):
    async def test_idle_connections_close_without_interrupting_active_requests(self):
        app = create_model_relay_app(
            sandbox_bearer_token="sandbox-token", worker_bearer_token="worker-token"
        )
        ...
        try:
            async with ClientSession() as client:
                active = asyncio.create_task(client.get(f"http://127.0.0.1:{port}/held"))
                ...
                self.assertFalse(active.done(), "active requests must outlive idle expiry")
                release.set()
