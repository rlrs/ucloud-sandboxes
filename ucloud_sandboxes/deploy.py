...
def render_remote_deploy_script(
    ...
    *,
    ...
    script_parts = [
        ...
        'RUNTIME_XFS_MODULE=""',
        'for module_candidate in "/lib/modules/$RUNTIME_KERNEL_RELEASE/kernel/fs/xfs"/xfs.ko*; do',
        '  if [ -f "$module_candidate" ]; then',
        '    RUNTIME_XFS_MODULE="$module_candidate"',
        "    break",
        "  fi",
        "done",
        ...
        '"$RUNTIME_KERNEL_RELEASE" "$RUNTIME_XFS_MODULE" <<\'PY\'',
        "import hashlib",
        "import gzip",
        "import io",
        "import json",
        ...
        "    }",
        "    if not xfs_module.is_file():",
        "        raise SystemExit('XFS kernel module is absent')",
        "    xfs_module_file = f'runtime/kernel/{kernel_release}/{xfs_module.name}'",
        "    manifest_payload['runtime']['kernel'] = {",
        "        'release': kernel_release,",
        "        'xfs_module': {",
        "            'file': xfs_module_file,",
        "            'sha256': sha256_file(xfs_module),",
        ...
        "                archive_paths.append((xfs_module, xfs_module_file))",
