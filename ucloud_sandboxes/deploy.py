...
def render_remote_deploy_script(
    ...
    *,
    ...
    script_parts = [
        ...
        'RUNTIME_XFS_MODULE="$(find "/lib/modules/$RUNTIME_KERNEL_RELEASE/kernel/fs" -type f -name \'xfs.ko*\' -print -quit)"',
        'RUNTIME_OVERLAY_MODULE="$(find "/lib/modules/$RUNTIME_KERNEL_RELEASE/kernel/fs" -type f -name \'overlay.ko*\' -print -quit)"',
        ...
        '  [ -n "$RUNTIME_XFS_MODULE" ] || return 1',
        '  [ -n "$RUNTIME_OVERLAY_MODULE" ] || return 1',
        ...
        '"$RUNTIME_KERNEL_RELEASE" "$RUNTIME_XFS_MODULE" '
        '"$RUNTIME_OVERLAY_MODULE" <<\'PY\'',
        "import hashlib",
        "import gzip",
        "import io",
        "import json",
        ...
        "xfs_module = Path(sys.argv[14])",
        "overlay_module = Path(sys.argv[15])",
        ...
        "    }",
        "    kernel_modules = {'xfs': xfs_module, 'overlay': overlay_module}",
        "    for module_name, module_path in kernel_modules.items():",
        "        if not module_path.is_file():",
        "            raise SystemExit(f'{module_name} kernel module is absent')",
        "    manifest_payload['runtime']['kernel'] = {",
        "        'release': kernel_release,",
        "        'modules': {",
        "            module_name: {",
        "                'file': f'runtime/kernel/{kernel_release}/{module_path.name}',",
        ...
        "                for module_name, module_path in kernel_modules.items():",
        "                    module_file = manifest_payload['runtime']['kernel']['modules'][module_name]['file']",
        "                    archive_paths.append((module_path, module_file))",
